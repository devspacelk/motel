...
class VikBooking
{
	...
	public static function displayCronParameters($file, $params = [])
	{
		...
		foreach ($config as $value => $cont) {
			...
			if (isset($cont['conditional']) && isset($config[$cont['conditional']]) && isset($params[$cont['conditional']])) {
				if (!$params[$cont['conditional']] || !strcasecmp($params[$cont['conditional']], 'off')) {
