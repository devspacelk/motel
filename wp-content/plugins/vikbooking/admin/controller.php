...
class VikBookingController extends JControllerVikBooking
{
	...
	private function do_updatebusy($callback = '')
	{
		 ...
		 * custom value "extra foreign data" (extracdata) is added.
		 * 
		 * @since 	1.16.0 (J) - 1.6.0 (WP)
		 */
		$pextracfk = VikRequest::getVar('extracfk', array());
		$pextracdata = VikRequest::getVar('extracdata', array(), 'request', 'array', VIKREQUEST_ALLOWRAW);
	...
	private function do_resendorderemail($cancellation = false)
	{
		...
		if (!empty($order['lang'])) {
			...
			if ($lang->getTag() != $order['lang']) {
				$lang->load('com_vikbooking', (defined('ABSPATH') && !defined('_JEXEC') ? VIKBOOKING_LANG : JPATH_ADMINISTRATOR), $order['lang'], true);
	...
	public function getbookingsinfo()
	{
		...
		foreach ($booking_infos as $k => $row) {
			...
			$str_checkout = date('d', $row['checkout']) . ' ' . VikBooking::sayMonth($stay_info_in['mon'], $short = true);
	...
	public function csvexportlaunch() {
		...
		if ($dbo->getNumRows() > 0) {
			...
			$orderscsv[] = array(
				...
				JText::translate('VBPVIEWORDERSONE'),
			...
			foreach ($orders as $kord => $ord) {
				...
				$orderscsv[] = array(
					$ord['id'],
					date(str_replace("/", $datesep, $df), $ord['ts']),
					date(str_replace("/", $datesep, $df), $booking_checkin),
					date(str_replace("/", $datesep, $df), $booking_checkout),
					$booking_nights,
					$ord['name'],
					$peoplestr,
					$custinfostr,
