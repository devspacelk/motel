...
function adjustHoveringTooltip() {
	setTimeout(function() {
		...
		if (tip_block.outerHeight() > table_wrap.outerHeight()) {
			// tooltip is too tall to fit in the table
		...
		if (otop < difflim) {
			...
			tip_block.css("bottom", "-="+(difflim - otop));
	...
	jQuery(document.body).on('click', 'td.busy, td.notbusy, td.busytmplock, td.subroom-busy', function(e) {
		...
		if (jQuery(this).hasClass('busy') || jQuery(this).hasClass('busytmplock') || jQuery(this).hasClass('subroom-busy')) {
			...
			if (jQuery(this).hasClass('subroom-busy')) {
				...
				room_name = jQuery('td.roomname[data-roomid="' + sub_room_data.split('-')[0] + '"]').not('.subroomname').first().find('.vbo-overview-roomname').text();
			} else {
				room_name = jQuery(this).parent('tr').find('td.roomname').find('.vbo-overview-roomname').text();
			}
				...
				(res) => {
					...
					try {
						...
						for (var b in obj_res) {
							...
							rday_booking_html += '				<span class="badge badge-' + (obj_res[b]['status'] == 'confirmed' ? 'success' : 'warning') + '">' + obj_res[b]['status_lbl'] + '</span>' + "\n";
